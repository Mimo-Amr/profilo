...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api/now-playing' && request.method === 'GET') {
      ...
      // Auto-refresh if token expires in < 60s
      let token = s.spotify_token;
      if (s.spotify_expires && Date.now() > Number(s.spotify_expires) - 60000) {
        token = await refreshToken(s.spotify_cid, s.spotify_refresh, env) || '';
        if (!token) return json({ playing: false, reason: 'token_expired' });
      }
...
async function refreshToken(clientId, refreshTok, env) {
  ...
  if (data.refresh_token) {
    batch.push(env.DB.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').bind('spotify_refresh', data.refresh_token));
  }
