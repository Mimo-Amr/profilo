  ...
  async fetch(request, env) {
    ...
    if (path === '/api/now-playing' && request.method === 'GET') {
      ...
      if (!s.lastfm_user || !s.lastfm_api_key) {
        return json({ playing: false, last_track: s.last_track||'', last_artist: s.last_artist||'', last_art: s.last_art||'' });
      }
      ...
      try {
        ...
        if (!lfRes.ok) return json({ playing: false, reason: `lastfm_${lfRes.status}`, last_track: s.last_track||'', last_artist: s.last_artist||'', last_art: s.last_art||'' });

        const data  = await lfRes.json();
        const track = data?.recenttracks?.track?.[0];
        if (!track) return json({ playing: false, last_track: s.last_track||'', last_artist: s.last_artist||'', last_art: s.last_art||'' });
        ...
        if (isPlaying) {
          ...
          return json({ playing: true, track: trackName, artist, art });
        } else {
          return json({ playing: false, last_track: trackName || s.last_track, last_artist: artist || s.last_artist, last_art: art || s.last_art });
        }
      } catch (e) {
        return json({ playing: false, last_track: s.last_track||'', last_artist: s.last_artist||'', last_art: s.last_art||'' });
      }
