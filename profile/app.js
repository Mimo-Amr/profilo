...
async function pollNowPlaying() {
  try {
    ...
    if (d && d.playing === true) {
      showMusicBar(d.track, d.artist, d.album, d.art);
    } else {
      hideSpotifyBar();
    }
  } catch (e) {
...
function showMusicBar(track, artist, album, artUrl) {
...
function initParticles() {
  ...
  particles = Array.from({ length: n }, () => ({
    ...
    vx: (Math.random()-.5)*.65, vy: (Math.random()-.5)*.65, r: Math.random()*1.8+.8,
  }));
}
function drawFrame() {
...
async function attemptLogin(){
  ...
  try {
    const u_hash=await sha256(user), p_hash=await sha256(pass);
...
document.getElementById('save-lastfm-btn').addEventListener('click',async()=>{
  ...
  if(r&&r.ok){
    toast('✓ Last.fm saved — music bar goes live now');
    pollNowPlaying();
  } else {
    toast('✕ Save failed');
  }
...
(async function init(){
  ...
  if(s){
    ...
    if(s.particle_color){particleColor=s.particle_color;_lastColor=s.particle_color;}
    if(s.bg_mode==='particles'||!s.bg_mode){/* already started */}
  }
  showPage();
