...
/* ═══════════════════════════════════════════════════════════
   CONFIG — empty string = same domain as the worker
═══════════════════════════════════════════════════════════ */
const WORKER_URL  = '';
...
async function apiPost(path, body) {
  ...
  if (!res.ok && res.status !== 401) throw new Error(`POST ${path} → ${res.status}`);
  return res.json();
}
...
async function saveSetting(updates) {
  ...
  try {
    return await apiPost('/api/settings', { ...sessionHashes, ...updates });
  } catch (e) {
    console.warn('[saveSetting]', e);
  }
}

/* ═══════════════════════════════════════════════════════════
   STATE POLLING — every 3s so all visitors see changes live
═══════════════════════════════════════════════════════════ */
let _lastName  = '';
let _lastColor = '';
...
function applyState(state) {
  if (state.display_name && state.display_name !== _lastName) {
    _lastName = state.display_name;
    document.getElementById('display-name').textContent = state.display_name;
  }
  if (state.particle_color && state.particle_color !== _lastColor) {
    _lastColor    = state.particle_color;
...
/* ═══════════════════════════════════════════════════════════
   SPOTIFY — poll /api/now-playing every 5 seconds
═══════════════════════════════════════════════════════════ */
...
function showSpotifyBar(track, artist, artUrl, ratio) {
  ...
  if (artUrl) { artEl.src = artUrl; artEl.style.display = 'block'; }
...
async function showSettingsView() {
  ...
  // Show the exact redirect URI the user needs to paste in Spotify dashboard
  const spRedirectEl = document.getElementById('sp-redirect-display');
  if (spRedirectEl) {
    spRedirectEl.textContent = `${window.location.origin}/spotify/callback`;
  }
  ...
  // Check Spotify connection status from D1
  try {
    const state = await apiGet('/api/state');
...
document.getElementById('update-name-btn').addEventListener('click', async () => {
  ...
  toast(res && res.ok ? '✓ Name saved — live for everyone' : '✕ Save failed');
});
...
document.getElementById('save-creds-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      toast('✕ Failed — re-login and try again');
    }
  } catch (e) {
...
/* ═══════════════════════════════════════════════════════════
   SPOTIFY CALLBACK RETURN CHECK
═══════════════════════════════════════════════════════════ */
function checkSpotifyReturn() {
  if (window.location.hash === '#spotify-connected') {
    history.replaceState(null, '', window.location.pathname);
    toast('✓ Spotify connected! Bar appears when music plays.');
  }
}
...
(async function init() {
  ...
  // Pull initial state from D1
  try {
    const state = await apiGet('/api/state');
    applyState(state);
    if (state.particle_color) particleColor = state.particle_color;
    if (state.bg_mode === 'particles' || !state.bg_mode) {
      // already started
    }
  } catch (e) {
    console.warn('[init]', e);
  ...
  // Poll remote state every 3s → live for all visitors
  setInterval(pollState, 3000);

  // Poll Spotify every 5s → show/hide bar automatically
  pollNowPlaying();
