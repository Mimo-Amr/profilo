...
/* ── API helpers — relative URLs, same domain as worker ── */
...
async function attemptLogin() {
  ...
  try {
    ...
    const res=await apiPost('/api/auth',{u_hash,p_hash});
    if(res&&res.ok) {
    ...
    errEl.textContent='✕ Cannot reach server — check console';
    errEl.style.display='block';
...
document.getElementById('sp-disconnect-btn').addEventListener('click',async()=>{
  try{await apiGet('/spotify/disconnect');}catch(e){}
...
(async function(){
  ...
  } catch(e){ console.warn('[init state]',e); }
