...
async function pollNowPlaying() {
  try {
    // POST is used intentionally — never cached by Cloudflare edge or browsers
...
(async function init() {
  ...
  if(s) {
    ...
    // Never read last_track from /api/state — only pollNowPlaying() controls the bar
    // This prevents any stale or cached track from painting on load
  }

  showPage();

  setInterval(pollState, 3000);
  // Poll immediately — bar only appears once real data arrives from Last.fm
  pollNowPlaying();
